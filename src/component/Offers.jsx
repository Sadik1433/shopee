      ...
      <div className="flex justify-evenly">
        <div className="flex flex-col gap-4 mx-auto overflow-visible ">
          {offers.map((offer, index) => (
            <div
              key={index}
              className={`${offer.bg} min-w-[600px]  text-white rounded-2xl px-5 py-2 border-3 shadow-lg flex flex-col`}
            >
              <div>
                <h3 className="text-2xl font-bold text-[var(--heading-color)]">{offer.title}</h3>
                <p className="text-1xl font-bold mt-1 ml-3 text-[var(--accent-color)] ">{offer.discount}</p>
